...
interface ShareVideoClientProps {
  videoId: string;
}

export default function ShareVideoClient({ videoId }: ShareVideoClientProps) {
  ...
  const [videoData, setVideoData] = useState<Playlist | null>(null);
  ...
  useEffect(() => {
    const handleSharedVideo = async () => {
      ...
      const existingPlaylist = playlistsData.find((p) => p.id === videoId);
      if (existingPlaylist) {
        setVideoData(existingPlaylist);
        setStatus("exists");
        // Navigate to playlist after short delay
        setTimeout(() => {
          router.replace(`/playlist/${videoId}`);
        }, 1500);
        return;
      }
      ...
      try {
        ...
        console.error("Error adding shared video:", err);
    ...
    handleSharedVideo();
  }, [videoId, authLoading, dispatch, router]);
